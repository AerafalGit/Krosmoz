...
public sealed class AchievementReward : IDatacenterObject<AchievementReward>
{
	...
	public static AchievementReward Deserialize(D2OClass d2OClass, BigEndianReader reader)
	{
		return new AchievementReward
		{
			...
			ItemsReward = d2OClass.Fields[4].AsList<uint>(reader, static (field, r) => field.AsUInt(r)),
			ItemsQuantityReward = d2OClass.Fields[5].AsList<uint>(reader, static (field, r) => field.AsUInt(r)),
			EmotesReward = d2OClass.Fields[6].AsList<uint>(reader, static (field, r) => field.AsUInt(r)),
			SpellsReward = d2OClass.Fields[7].AsList<uint>(reader, static (field, r) => field.AsUInt(r)),
			TitlesReward = d2OClass.Fields[8].AsList<uint>(reader, static (field, r) => field.AsUInt(r)),
			OrnamentsReward = d2OClass.Fields[9].AsList<uint>(reader, static (field, r) => field.AsUInt(r)),
		};
	}
}
