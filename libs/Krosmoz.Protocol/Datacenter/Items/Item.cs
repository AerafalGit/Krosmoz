...
public class Item : IDatacenterObject
{
	...
	public virtual void Deserialize(D2OClass d2OClass, BigEndianReader reader)
	{
		...
		RecipeIds = d2OClass.Fields[24].AsList(reader, static (field, r) => field.AsUInt(r));
		DropMonsterIds = d2OClass.Fields[25].AsList(reader, static (field, r) => field.AsUInt(r));
		...
		FavoriteSubAreas = d2OClass.Fields[28].AsList(reader, static (field, r) => field.AsUInt(r));
