...
public sealed class Recipe : IDatacenterObject<Recipe>
{
	...
	public static Recipe Deserialize(D2OClass d2OClass, BigEndianReader reader)
	{
		return new Recipe
		{
			ResultId = d2OClass.Fields[0].AsInt(reader),
			ResultLevel = d2OClass.Fields[1].AsInt(reader),
			IngredientIds = d2OClass.Fields[2].AsList<int>(reader, static (field, r) => field.AsInt(r)),
			Quantities = d2OClass.Fields[3].AsList<uint>(reader, static (field, r) => field.AsUInt(r)),
		};
	}
}
