...
/// </summary>
/// <typeparam name="TArgument">The reference type of the argument to convert.</typeparam>
/// <typeparam name="TContext">The type of the command context.</typeparam>
public sealed class NullableArgumentConverter<TArgument, TContext> : IArgumentConverter<TArgument?, TContext>
    where TArgument : class
    where TContext : CommandContext
{
    /// <summary>
    /// Converts a nullable reference type argument from the command context.
    /// </summary>
    /// <param name="context">The context in which the command is executed, containing the argument to convert.</param>
    /// <param name="cancellationToken">A token to monitor for cancellation requests.</param>
    /// <returns>
    /// A <see cref="ValueTask{TResult}"/> containing the converted nullable reference type argument of type <typeparamref name="TArgument"/>
    /// if successful, or <c>null</c> if the conversion fails.
    /// </returns>
    public async ValueTask<TArgument?> ConvertAsync(TContext context, CancellationToken cancellationToken)
    {
        ...
        if (context.Converters.TryGetValue(typeof(TArgument).Name, out var converter) && converter is IArgumentConverter<TArgument, TContext> typedConverter)
            result = await typedConverter.ConvertAsync(context, cancellationToken).ConfigureAwait(false);
