...
public sealed class ClassParser : IParser<ClassSymbol>
{
    ...
    private static void ParseMethods(ClassSymbol classSymbol)
    {
        ...
        ParseMethodCondition(classSymbol);
    ...
    /// <summary>
    /// Parses condition-related methods from the class symbol's metadata and updates the properties
    /// with the corresponding minimum and maximum condition values.
    /// </summary>
    /// <param name="classSymbol">The class symbol to populate with parsed condition methods.</param>
    private static void ParseMethodCondition(ClassSymbol classSymbol)
    {
        foreach (var match in RegexStorage.PropertyConditionMin().Matches(classSymbol.Metadata.Source).Cast<Match>())
        {
            if (!EnsurePropertyExist(classSymbol, match, out var property))
                continue;

            if (match.Groups.TryGetValue("minValue", out var minValueGroup) && minValueGroup.Value.All(char.IsDigit))
                property.ConditionMin = int.Parse(minValueGroup.Value);
        }

        foreach (var match in RegexStorage.PropertyConditionMinAndMax().Matches(classSymbol.Metadata.Source).Cast<Match>())
        {
            if (!EnsurePropertyExist(classSymbol, match, out var property))
                continue;
            ...
            if (match.Groups.TryGetValue("maxValue", out var maxValueGroup) && maxValueGroup.Value.All(char.IsDigit))
                property.ConditionMax = int.Parse(maxValueGroup.Value);
        }

        foreach (var match in RegexStorage.PropertyVectorConditionMin().Matches(classSymbol.Metadata.Source).Cast<Match>())
        {
            if (!EnsurePropertyExist(classSymbol, match, out var property))
                continue;
